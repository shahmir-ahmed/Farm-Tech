...
class CartServices {
  // add item to cart collection with buyer id
      ...
      await FirebaseFirestore.instance
        ...
        if (snapshot.docs.isNotEmpty) {
          ...
          final updatedCartItemModel = CartItemModel(
              quantity: newQuantity.toString(), total: newTotal.toString());
  ...
  Stream<List<CartItemModel>?>? getBuyerCartItemsStream(BuyerModel buyerModel) {
    try {
      return FirebaseFirestore.instance.collection('carts')
        .where('buyerId', isEqualTo: buyerModel.docId).snapshots().map(
